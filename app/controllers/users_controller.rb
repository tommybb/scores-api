class UsersController < ApplicationController
  ...
  def create
    if user.save
      redirect_to user_path(user), notice: t('user.user_created')
    else
      render :new
    end
  end
  ...
  def update
    if user.update(user_params)
      redirect_to user_path(user), notice: t('user.user_updated')
    else
      render :edit
    end
  end
  ...
  def destroy
    user.destroy
    redirect_to users_path, notice: t('user.user_destroyed')
  end
